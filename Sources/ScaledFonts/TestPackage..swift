...
extension NSNotification.Name {
    public static let FontMetricsContentSizeCategoryDidChange: NSNotification.Name =
        Notification.Name("FontMetricsContentSizeCategoryDidChange")
}
...
/// sizeCategory: UIContentSizeCategory to override the system Accessibility settings for uni
/// testing. Manually adjusting the accessibility dynamic text slider will reset the manually
/// assigned sizeCategory value.
///
public class FontMetricsss {

    /// By declaring `default` as a static let, we will be able to use a globally consistent
    /// instance of `FontMetrics`. It behaves similarly to `NotificationCenter.default` or
    /// `UIDevice.current`.
    public static let `default` = FontMetricsss()
    ...
    /// Initializer will take the system's `preferredContentSizeCategory` at startup if it is not
    /// provided. Therefore, `FontMetrics.default` will be initialized with the system's current
    /// Dynamic Type setting.
    @available(iOS 10.0, *)
    public init(withSizeCategory sizeCategory: UIContentSizeCategory =
    ...
    /// By using a delegate instead of directly changing the `sizeCategory` here, a user of
    /// `FontMetrics` can later define custom behavior when the system's Dynamic Type setting
    /// changes instead of being forced to change the class itself.
    ...
    private var scaler: CGFloat {
        return UIFont.preferredFont(
            ...
            compatibleWith: UITraitCollection(
                preferredContentSizeCategory: self.sizeCategory
            )).pointSize / 17.0
    }
...
extension FontMetricsss: UIContentSizeCategoryDelegate {
